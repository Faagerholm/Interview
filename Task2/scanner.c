...
void packet_handler(u_char *args, const struct pcap_pkthdr *packet_header, const u_char *packet){
	...
	/* Get information */
	bssid = packet + 34; 	// Transmitter address can be found on offset 34
	...
	/*
	fprintf(stdout, "%i ",packet_counter++);
...
*/
		char adress_str[17];
		sprintf(adress_str, "%02x:%02x:%02x:%02x:%02x:%02x", bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5]);
		...
		json_object_add_object(&object, "address",adress_str);
		json_object_send(&object);
	//TODO: json_object_save();	
	}	
	return;
}
